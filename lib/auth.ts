...
export function authConfig() {
  ...
  if (!password || password.length < 12 || password.startsWith("replace-with-")) throw new Error("AUTH_PASSWORD must be a unique password of at least 12 characters");
  if (!secret || secret.length < 32 || secret.startsWith("replace-with-")) throw new Error("AUTH_SECRET must contain at least 32 random characters");
...
export async function verifySession(value: string | undefined, now = Date.now()) {
  ...
  if (!expires || !suppliedSignature || extra || !/^\d+$/.test(expires) || Number(expires) <= Math.floor(now / 1000)) return false;
  const { secret } = authConfig();
  return secureEqual(encoder.encode(suppliedSignature), encoder.encode(await signature(expires, secret)));
}
